...
#ADD beldex /opt/android/beldex
RUN cd /opt/android \
     && apt-get install -y libevent-dev libunbound-dev libsystemd-dev systemd \
     && apt update \
     && apt install -y build-essential cmake pkg-config libboost-all-dev libssl-dev libzmq3-dev libunbound-dev libsodium-dev libunwind8-dev liblzma-dev libreadline6-dev libldns-dev libexpat1-dev doxygen graphviz libpgm-dev libsqlite3-dev libcurl4-openssl-dev \
     && cd ../ \
     && git clone --recursive  https://github.com/victor-tucci/unbound.git \
     && cd unbound \
...
ADD  . /opt/android/beldex_andriod
RUN  cd /opt/android/beldex_andriod \
     && rm -rf build \
     && mkdir -p build/release \
     && CMAKE_INCLUDE_PATH="${PREFIX}/include" \
       CMAKE_LIBRARY_PATH="${PREFIX}/lib" \
       ANDROID_STANDALONE_TOOLCHAIN_PATH=${TOOLCHAIN_DIR} \
       USE_SINGLE_BUILDDIR=1 ./build-all-arch.sh
